...
public final class EntityUtils2 extends MinecraftInstance {
            ...
            if (targetInvisible || !entity.isInvisible()) {
                if (targetPlayer && (entity instanceof EntityPlayer)) {
                    ...
                    if (canAttackCheck) {
                        ...
                        if (isFriend(entityPlayer) && !DarkNya.moduleManager.getModule(NoFriends.class).getState())
                            return false;

                        if (entityPlayer.isSpectator())
                            return false;

                        final Teams teams = (Teams) DarkNya.moduleManager.getModule(Teams.class);
    ...
    public static boolean isFriend(final Entity entity) {
        return (entity instanceof EntityPlayer) && entity.getName() != null &&
                DarkNya.fileManager.friendsConfig.isFriend(ColorUtils.stripColor(entity.getName()));
    }
