...
class LocalAutoSettingsCommand : Command("localautosettings", "localsetting", "localsettings", "localconfig") {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val scriptFile = File(DarkNya.fileManager.settingsDir, args[2])
                        ...
                        if (scriptFile.exists()) {
                            try {
                                ...
                                DarkNya.hud.addNotification(Notification("Updated Settings","Updated Settings", NotifyType.INFO))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
    ...
    private fun getLocalSettings(): Array<File>? = DarkNya.fileManager.settingsDir.listFiles()
}