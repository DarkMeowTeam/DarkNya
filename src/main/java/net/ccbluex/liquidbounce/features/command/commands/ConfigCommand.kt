...
class ConfigCommand : Command("config") {
    override fun execute(args: Array<String>) {
        if (args.size >= 2) {
            ...
            when (args[1].toLowerCase()) {
                        ...
                        try {
                            ...
                            while (iterator.hasNext()) {
                                ...
                                if (key.equals("CommandPrefix", ignoreCase = true)) {
                                    DarkNya.commandManager.prefix = value.asCharacter
                                    ...
                                    val module = DarkNya.moduleManager.getModule(key)
                                    if (module != null) {
                ...
                "delete" -> {
                    if (args.size > 2) {
                        val file = File(DarkNya.fileManager.configsDir, "${args[2]}.json")
                        if (file.exists()) {
                            try {
                ...
                "save" -> {
                    if (args.size > 2) {
                        try {
                            ...
                            jsonObject.addProperty("CommandPrefix", DarkNya.commandManager.prefix)
                            ...
                            DarkNya.moduleManager.modules.stream()
                            ...
                            val printWriter =
                                PrintWriter(FileWriter(File(DarkNya.fileManager.configsDir, "${args[2]}.json")))
                ...
                "folder" -> {
                    try {
                        Desktop.getDesktop().open(DarkNya.fileManager.configsDir)
                        chat("已打开配置文件夹.")
                        return
                    } catch (t: Throwable) {
    ...
    private fun getLocalSettings(): Array<File>? = DarkNya.fileManager.configsDir.listFiles()
