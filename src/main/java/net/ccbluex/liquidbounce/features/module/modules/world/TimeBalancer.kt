...
class TimeBalancer : Module() {
    ...
    */
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (m == time.get()) {
            timr = true
            (mc.timer as IMixinTimer).timerSpeed = timer.get()
            m = 0
            s = 0
        }
        if (s == tie.get()) {
        ...
        if (counterMode) {
            ...
            when (setFrame.get().toLowerCase()) {
                "^" -> {
                    ...
                    GlStateManager.resetColor()
                    FontLoaders.F24.drawString("Timer：$m ReleaseTime：$s", (startX - 4 + 36).toFloat(), (startY + 7.5).toFloat(), Color(fontR.get(),fontG.get(),fontB.get(),fontA.get()).rgb)
                    GlStateManager.resetColor()
                }
                "<" -> {
                    RoundedUtil.drawRound(startX.toFloat(), startY.toFloat(), 3f, 22f, 3F, Color(frameR.get(), frameG.get(),frameB.get(), frameA.get()))
                    GlStateManager.resetColor()
                    FontLoaders.F24.drawString("Timer：$m ReleaseTime：$s", (startX - 4 + 36).toFloat(), (startY + 6.5).toFloat(), Color(fontR.get(),fontG.get(),fontB.get(),fontA.get()).rgb)
                    GlStateManager.resetColor()
                }
            }
            GlStateManager.resetColor()
