...
class GrimVelocity : AntiKBMode("GrimAC") {
    ...
    override fun onTick(event: TickEvent){
        ...
        if ((DarkNya.moduleManager[AntiKnockback::class.java] as AntiKnockback).sendC03Value.get()) {
        ...
        if ((DarkNya.moduleManager[AntiKnockback::class.java] as AntiKnockback).c0fTestValue.get()){
            connection.sendPacket(CPacketConfirmTransaction())
        }

        if ((DarkNya.moduleManager[AntiKnockback::class.java] as AntiKnockback).breakValue.get()) {
