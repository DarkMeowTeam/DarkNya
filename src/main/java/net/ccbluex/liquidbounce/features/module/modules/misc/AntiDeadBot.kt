...
class AntiDeadBot : Module() {
    ...
    private fun botchange(name: String, cooldown: Long) {
        bots++
        DarkNya.fileManager.friendsConfig.addFriend(name)
        if (msg.get()) {
            logString(name, "add")
        }
        Thread {
            try {
                ...
                DarkNya.fileManager.friendsConfig.removeFriend(name)
                bots--
                if (msg.get()) {
            ...
            when (mode.toLowerCase()) {
                "add" -> {
                    when (logMode.get().toLowerCase()) {
                        "old" -> "§8[§c§l${DarkNya.CLIENT_NAME}提醒您§8] §d添加无敌人：§7$name"
                        "fdpclient" -> "§7[§cAntiBot§7] §fAdded §7$name§f due to it being a bot."
                        "nullclient" -> "§7[§cAntiBots§7] §fAdded a bot(§7$name§f)"
                        "wawa" -> "§6${DarkNya.CLIENT_NAME} §7=» §f玩家 §7$name §f狂暴死去"
                        else -> "§8[§c§l${DarkNya.CLIENT_NAME}提醒您§8] §d添加无敌人：§7$name"
                    }
                }
                "remove" -> {
                    when (logMode.get().toLowerCase()) {
                        "old" -> ("§8[§c§l${DarkNya.CLIENT_NAME}提醒您§8] §d删除无敌人：§7$name")
                        "fdpclient" -> ("§7[§cAntiBot§7] §fRemoved §7$name§f due to respawn.")
                        "nullclient" -> ("§7[§cAntiBots§7] §fRemoved a bot(§7$name§f)")
                        "wawa" -> "§6${DarkNya.CLIENT_NAME} §7=» §f玩家 §7$name §f炸裂归来"
                        else -> "§8[§c§l${DarkNya.CLIENT_NAME}提醒您§8] §d添加无敌人：§7$name"
                    }
                }
                else -> ""
    ...
    private fun clearAll() {
        DarkNya.fileManager.friendsConfig.clearFriends()
    }
