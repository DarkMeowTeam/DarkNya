...
class KillAura : Module() {
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if (event.eventState == EventState.PRE) {
            update()
            val strafeFix = DarkNya.moduleManager[StrafeFix::class.java] as StrafeFix
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (circleValue.get()) {
            ...
            GL11.glVertex2f(cos(360 * Math.PI / 180.0).toFloat() * range, (sin(360 * Math.PI / 180.0).toFloat() * range))
...
//                GL11.glPopMatrix()
    ...
    private fun isEnemy(entity: Entity?): Boolean {
        if ((entity is EntityLivingBase) && entity != null && (EntityUtils.targetDead || isAlive(entity)) && entity != mc.player) {
            ...
            if (EntityUtils.targetPlayer && (entity is EntityPlayer)) {
                ...
                if (player.isClientFriend() && !DarkNya.moduleManager[NoFriends::class.java].state)
                    return false

                val teams = DarkNya.moduleManager[Teams::class.java] as Teams
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        // Call attack event
        DarkNya.eventManager.callEvent(AttackEvent(entity))
        ...
        val criticals = DarkNya.moduleManager[Criticals::class.java] as Criticals
    ...
    private fun updateHitable() {
        ...
        if (raycastValue.get()) {
            ...
            if (raycastValue.get() && raycastedEntity != null && raycastedEntity is EntityLivingBase
                && (DarkNya.moduleManager[NoFriends::class.java].state || !(raycastedEntity is EntityPlayer && raycastedEntity.isClientFriend())))
