...
class GuiHudDesigner : GuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        DarkNya.hud.render(true)
        DarkNya.hud.handleMouseMove(mouseX, mouseY)

        if (!DarkNya.hud.elements.contains(selectedElement))
        ...
        if (wheel != 0) {
            for (element in DarkNya.hud.elements) {
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        DarkNya.hud.handleMouseClick(mouseX, mouseY, mouseButton)
    ...
    override fun mouseReleased(mouseX: Int, mouseY: Int, state: Int) {
        ...
        DarkNya.hud.handleMouseReleased()
    }

    override fun onGuiClosed() {
        Keyboard.enableRepeatEvents(false)
        DarkNya.fileManager.saveConfig(DarkNya.fileManager.hudConfig)
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        when (keyCode) {
            Keyboard.KEY_DELETE -> if (Keyboard.KEY_DELETE == keyCode && selectedElement != null)
                DarkNya.hud.removeElement(selectedElement!!)
            ...
            else -> DarkNya.hud.handleKey(typedChar, keyCode)
        }
