...
class Text2(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    companion object {
        ...
        fun defaultClient(): Text2 {
            ...
            text.displayString.set( DarkNya.CLIENT_NAME + " |" + "Fps:%fps% ")
    ...
    private var displayText: String = DarkNya.CLIENT_NAME
    val hud = DarkNya.moduleManager[HUD::class.java] as HUD
    //val username: String = mc.player!!.name
    private val display: String
        get() {
            val textContent = if (displayString.get().isEmpty() && !editMode)
                DarkNya.CLIENT_NAME + " | Dev: WaWa"
            else
                displayString.get()
    ...
    private fun getReplacement(str: String): String? {
        ...
        return when (str) {
            "clientname" -> DarkNya.CLIENT_NAME
            "clientversion" -> "b${DarkNya.CLIENT_VERSION}"
            "clientcreator" -> DarkNya.CLIENT_CREATOR
            ...
            "clientName" -> DarkNya.CLIENT_NAME
            "clientVersion" -> DarkNya.CLIENT_VERSION
            "clientCreator" -> DarkNya.CLIENT_CREATOR
            "fps" -> Minecraft.getDebugFPS().toString()
            "date" -> DATE_FORMAT.format(System.currentTimeMillis())
            "time" -> HOUR_FORMAT.format(System.currentTimeMillis())
