...
public class GuiAltManager extends GuiScreen {
    ...
    public static void loadGenerators() {
        try {
            ...
            final JsonElement jsonElement = new JsonParser().parse(HttpUtils.get(DarkNya.CLIENT_CLOUD + "/generators.json"));
    ...
    public static String login(final MinecraftAccount minecraftAccount) {
        ...
        if (result == LoginUtils.LoginResult.LOGGED) {
            ...
            DarkNya.fileManager.saveConfig(DarkNya.fileManager.accountsConfig);
    ...
    public void initGui() {
        ...
        for (int i = 0; i < DarkNya.fileManager.accountsConfig.getAccounts().size(); i++) {
            MinecraftAccount minecraftAccount = DarkNya.fileManager.accountsConfig.getAccounts().get(i);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.font35.drawCenteredString(this.searchField.getText().isEmpty() ? (DarkNya.fileManager.accountsConfig.getAccounts().size() + " Alts") : this.altsList.accounts.size() + " Search Results", width / 2.0f, 18, 0xffffff);
        ...
        switch (button.id) {
            ...
            case 2:
                if (altsList.getSelectedSlot() != -1 && altsList.getSelectedSlot() < altsList.getSize()) {
                    DarkNya.fileManager.accountsConfig.removeAccount(this.altsList.accounts.get(altsList.getSelectedSlot()));
                    DarkNya.fileManager.saveConfig(DarkNya.fileManager.accountsConfig);
            ...
            case 7:
                ...
                while ((line = bufferedReader.readLine()) != null) {
                    ...
                    if (!DarkNya.fileManager.accountsConfig.accountExists(accountData[0])) {
                        if (accountData.length > 1)
                            DarkNya.fileManager.accountsConfig.addAccount(accountData[0], accountData[1]);
                        else
                            DarkNya.fileManager.accountsConfig.addAccount(accountData[0]);
                    }
                }
            ...
            case 11:
                ...
                break;
            case 12:
                if (DarkNya.fileManager.accountsConfig.getAccounts().size() == 0) {
                    status = "§cThe list is empty.";
                    return;
                }
                ...
                try {
                    ...
                    for (MinecraftAccount account : DarkNya.fileManager.accountsConfig.getAccounts()) {
    ...
    private class GuiList extends GuiSlot {
        ...
        private void updateAccounts(String search) {
            if (search == null || search.isEmpty()) {
                this.accounts = DarkNya.fileManager.accountsConfig.getAccounts();
                return;
            }
