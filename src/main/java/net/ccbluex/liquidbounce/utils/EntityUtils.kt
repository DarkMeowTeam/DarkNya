...
object EntityUtils : MinecraftInstance() {
            ...
            if (targetInvisible || !entity.isInvisible) {
                if (targetPlayer && (entity is EntityPlayer)) {
                    ...
                    if (canAttackCheck) {
                        ...
                        if (entityPlayer.isClientFriend() && !DarkNya.moduleManager.getModule(NoFriends::class.java).state)
                            return false

                        if (entityPlayer.isSpectator) return false
                        val teams = DarkNya.moduleManager.getModule(Teams::class.java) as Teams
    ...
    fun isFriend(entity: Entity): Boolean {
        return entity is EntityPlayer && DarkNya.fileManager.friendsConfig.isFriend(stripColor(entity.name))
    }
