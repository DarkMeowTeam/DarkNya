...
class Script(val scriptFile: File) : MinecraftInstance() {
    ...
    init {
        ...
        scriptEngine.put("moduleManager", DarkNya.moduleManager)
        scriptEngine.put("commandManager", DarkNya.commandManager)
        scriptEngine.put("scriptManager", DarkNya.scriptManager)
    ...
    inner class RegisterScript : java.util.function.Function<JSObject, Script> {
        /**
    ...
    fun registerModule(moduleObject: JSObject, callback: JSObject) {
        ...
        DarkNya.moduleManager.registerModule(module)
    ...
    fun registerCommand(commandObject: JSObject, callback: JSObject) {
        ...
        DarkNya.commandManager.registerCommand(command)
    ...
    private fun supportLegacyScripts() {
        if (getMagicComment("api_version") != "2") {
            ...
            val legacyScript = DarkNya::class.java.getResource("/assets/minecraft/liquidbounce/scriptapi/legacy.js").readText()
    ...
    fun onDisable() {
        ...
        registeredModules.forEach { DarkNya.moduleManager.unregisterModule(it) }
        registeredCommands.forEach { DarkNya.commandManager.unregisterCommand(it) }
    ...
    fun import(scriptFile: String) {
        val scriptText = File(DarkNya.scriptManager.scriptsFolder, scriptFile).readText()
